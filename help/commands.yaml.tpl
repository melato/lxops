    ...
    commands:
      ...
      convert:
        ...
        long: |
          ...
          The conversion is done by running the commands:
          ...
          The command can also update the metadata.yaml file, with option values.
          This is a temporary feature that I need because the metadata in my
          images cause problems with the aliases generated by
          incus-simplestreams.

          configwill be removed once I start the metadata
          This may be necessary if the metadata values were not carefully set
          in the image earlier.
                    
          The -parse option specifies a format that will be used
          to parse the image name and extract image metadata from it.
          
          In this command, the image name is the last component of the path
          (minus the .tar.gz suffix).
          In the export command, the image name is the image alias specified in the command line.
          
          All formats expect the name to be in the format {prefix}-{name}-{date-time}
          Example: a-nginx-20240515-2251
          The following parse formats are supported:
            ovr: os={prefix} variant={name} release={date-time}
            ovd: os={prefix} variant={name} date={date-time}
            vod: variant={prefix} os={name} date={date-time}          
