...
type User struct {
	...
	Sudo bool `yaml:"sudo"`
	// If Ssh is true and AuthorizedKeys is not specified,
	// copy the current host user's ~.ssh/authorized_keys to this user's ~/.ssh/authorized_keys
	// If AuthorizedKeys is specified, then Ssh is not used.
	Ssh bool `yaml:"ssh"`
